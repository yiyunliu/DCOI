...
Lemma wne_var Ξ ℓ : wne tAbsurd /\ IOk Ξ ℓ tAbsurd.
...
Proof.
  ...
  - move => ℓ0 A B PA PF hPA ihPA hTot hRes ihPF.
    have hzero : PA ℓ0 tAbsurd by hauto l:on use:wne_var.
    split.
    split.
    ...
    + apply wn_pi.
      ...
      move /ihPF : (hzero) hPB => /[apply].
      move => [[_ _]h].
      apply wn_antirenaming with (ξ := (tAbsurd..))=>//.
      case => //=.
  - hauto l:on.
  - hauto lq:on db:nfne.
  ...
  - move => ℓ0 A B PA PF hPA ihPA hTot hRes ihPF.
    ...
    + apply wn_sig.
      sfirstorder.
      have : PA ℓ0 tAbsurd by hauto l:on use:wne_var.
