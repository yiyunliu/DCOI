...
Lemma wne_absurd :
  wne tAbsurd.
Proof.
  hauto lq:on ctrs:rtc unfold:wne.
Qed.
...
Lemma ext_wn ℓ0 (a : tm) :
    wn (tApp a ℓ0 tAbsurd) ->
    wn a.
Proof.
  move E : (tApp a ℓ0 tAbsurd) => a0 [v [hr hv]].
  move : a E.
  move : hv.
  elim : a0 v / hr.
  ...
  - move => a0 a1 a2 hr0 hr1 ih hnfa2.
    ...
    + move => ? a0 a3 b2 b3 ℓ1 ? ? [? ? ?] ? [? ? ?]. subst.
      have ? : b3 = tAbsurd by hauto lq:on inv:Par. subst.
      suff : wn (tAbs ℓ a3) by hauto lq:on ctrs:Par, rtc unfold:wn.
      have : wn (subst_tm (tAbsurd ..) a3) by sfirstorder.
